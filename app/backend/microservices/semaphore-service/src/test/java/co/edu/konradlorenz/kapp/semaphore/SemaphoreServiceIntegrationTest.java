...
class SemaphoreServiceIntegrationTest {
    ...
    void rejectsAnonymous() throws Exception {
        mockMvc.perform(get("/api/semaphore/ping"))
                .andExpect(status().isUnauthorized())
                // The shared ApiError envelope, produced by the entry point in common.
                .andExpect(jsonPath("$.status").value(401))
                .andExpect(jsonPath("$.path").value("/api/semaphore/ping"))
    ...
    void acceptsValidToken() throws Exception {
        mockMvc.perform(get("/api/semaphore/ping")
                ...
                .andExpect(jsonPath("$.service").value("semaphore-service"))
    ...
    void unknownPathUsesSharedEnvelope() throws Exception {
        mockMvc.perform(get("/api/semaphore/does-not-exist")
